...
class WindowStateMatcher {
    ...
    _handleOccupiedWindowModified(winid, coreDetails, currentSlot, modifiedEvent) {
        ...
        // only migrate to a better slot during the initial matching phase, not
        // for long-running TRACKING windows where title changes are normal app
        // behavior (switching tabs, documents, etc.)
        const isLongTermTracking = windowState.state === WindowState.TRACKING &&
            (Date.now() - windowState.transitionTime > this._config.GENERIC_TITLE_EXTENDED_WAIT)

        if (!isLongTermTracking && this.hasTitleBecomeMoreSpecific(oldTitle, newTitle)) {
